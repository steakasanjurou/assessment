(function () {
    'use strict';
    const userNameInput = document.getElementById('user-name'); //htmlエレメント。入力欄のテキスト。
    const assessmentButton = document.getElementById('assessment');//htmlエレメント。ボタン。
    const resultDivided = document.getElementById('result-area');//htmlエレメント。診断結果。
    const tweetDivided = document.getElementById('tweet-area');//htmlエレメント。ツイートボタン
    ...
    assessmentButton.onclick = () => {
        ...
        if (userName.length === 0) { //入力欄が空白の時は処理を終了する
            return;
        }

        //診断結果表示エリアの作成
        removeAllChildren(resultDivided); //診断結果のデータが何個も連なっているときは、先頭のデータ以外は抹殺する。
        ...
        //診断結果表示エリアの作成
        removeAllChildren(tweetDivided); //診断結果のデータが何個も連なっているときは、先頭のデータ以外は抹殺する。
    ...
    userNameInput.onkeydown = (event) => {
        if (event.keyCode === 13) {
            ...
            assessmentButton.onclick(); //ボタンをクリックしたときの無名関数だが、左のように書けば関数のように扱える。
        }
    };
    ...
    const answers = [
        ...
        '{userName}のいいところは厳しさです。{userName}の厳しさがものごととをいつも成功に導きます。',
    ...
    console.assert(assessment('太郎') === assessment('太郎'),
        '診断結果ですが、同じ名前であるにもかかわらず違う結果が出てしまっているため、おかしいです。');
